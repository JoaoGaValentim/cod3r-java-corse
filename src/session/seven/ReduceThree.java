...
public class ReduceThree {
        public static void main(String[] args) {
                ...
                Predicate<Student> startsWithAAndIsApprove = student -> student.getName().startsWith("A")
                                && student.getAverage() > 6.9;

                Function<Student, Double> averages = student -> student.getAverage();
                BiFunction<Average, Double, Average> averagesSum = (average, value) -> average.add(value);
                BinaryOperator<Average> combiner = (fAverage, lAverage) -> Average.combine(fAverage, lAverage);

                var average = students
                                .stream()
                                .filter(startsWithAAndIsApprove)
                                .map(averages)
                                .reduce(new Average(), averagesSum, combiner);

                System.out.println("Média dos alunos da letra A => " + average.getTotal() / average.getQuantity());
        }
}
